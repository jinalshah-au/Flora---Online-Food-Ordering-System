...
//         echo "<script> window.location='editDriverById.php' </script>";
//     }
// }else{
...
if(isset($_POST['submit'])){
    ...
    if(isEmpty($name) || isEmpty($phone) || isEmpty($license_no)){
        echo "<script> alert('All fields are mandatory.')</script>";
    }else{
        $driver = new Driver($id,$name,$phone,$license_no);
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col" style="margin:10px;padding:10px;">
                    Name :
                    <input type="text" value="<?php echo $driver->getName(); ?>" name="name" id="title" class="form-control" style="margin:10px;" placeholder="title">
                </div>
                <div class="col" style="margin:10px;padding:10px;">
                    Phone :
                    <input type="text" value="<?php echo $driver->getPhone(); ?>" name="phone" id="description" class="form-control" style="margin:10px;" placeholder="description">
                </div>
            </div>
            ...
            <div class="row">
                <div class="col" style="margin:10px;padding:10px;">
                    License :
                    <input type="text" value="<?php echo $driver->getLicenseNo(); ?>" name="license_no" class="form-control" style="margin:10px;" placeholder="price">
                </div>
            </div>
